...
// Extended user interface with user_role
interface ExtendedAdapterUser extends AdapterUser {
	user_role?: string;
}

interface NeonUser extends ExtendedAdapterUser {
	accounts: {
		provider: string;
...
interface NeonAdapter extends Adapter {
	createUser(data: Omit<ExtendedAdapterUser, 'id'>): Promise<ExtendedAdapterUser>;
	getUser(userId: string): Promise<ExtendedAdapterUser | null>;
	...
	return {
      ...
      RETURNING identifier, expires, token `;
		...
		async createUser(user: Omit<ExtendedAdapterUser, 'id'>) {
			...
			const sql = `
        INSERT INTO auth_users (name, email, "emailVerified", image, user_role)
        VALUES ($1, $2, $3, $4, $5)
        RETURNING id, name, email, "emailVerified", image, user_role`;
		...
		async updateUser(user: Partial<ExtendedAdapterUser>): Promise<ExtendedAdapterUser> {
        ...
        where id = $1
        RETURNING name, id, email, "emailVerified", image, user_role
      `;
