  ...
  return (
    <div className="flex sm:w-full w-screen sm:min-w-[850px] flex-col">
      <div className="flex w-full items-center gap-2 p-5">
        ...
        <div className="flex flex-row divide-x divide-neutral-200 rounded-[8px] h-8 w-[300px] border border-neutral-200 bg-neutral-50 text-neutral-500">
      ...
      <div className="flex flex-grow flex-col items-center justify-center pt-[100px] text-center gap-[20px]">
        <h1 className="text-4xl font-medium text-neutral-900 px-2">
          Uh-oh! This page doesn't exist (yet).
        </h1>

        <p className="pt-4 pb-12 px-2 text-neutral-500">
          Looks like "<span className="font-bold">/{missingPath}</span>" isn't part of your project.
          But no worries, you've got options!
        </p>

        <div className="px-[20px] w-full">
          <div className="flex flex-row justify-center items-center w-full max-w-[800px] mx-auto border border-neutral-200 rounded-lg p-[20px] mb-[40px] gap-[20px]">
        ...
        {siteMap ? (
          <div className="flex flex-col justify-center items-center w-full px-[50px]">
            <div className="flex flex-col justify-between items-center w-full max-w-[600px] gap-[10px]">
              <p className="text-sm text-neutral-300 pb-[10px] self-start p-4">PAGES</p>
              {siteMap.webPages?.map((route) => (
                <button
                  type="button"
                  onClick={() => handleSearch(route.cleanRoute || '')}
                  key={route.id}
                  className="flex flex-row justify-between text-center items-center p-4 rounded-lg bg-white shadow-sm w-full hover:bg-neutral-50"
                >
                  <h3 className="font-medium text-neutral-900">{route.name}</h3>
                  <p className="text-sm text-neutral-400">{route.cleanRoute}</p>
                </button>
              ))}
            </div>
          </div>
                ...
                <div className="w-full flex-1 flex flex-col items-center ">
                  <div className="relative w-full max-w-[350px] h-48 sm:h-56 lg:h-64 overflow-hidden rounded-[8px] border border-comeback-gray-75 transition-all group-hover:shadow-md">
                    <button
                      ...
                      className="h-full w-full rounded-[8px] bg-neutral-50 bg-cover"
                    />
                  </div>
                  <p className="pt-3 text-left text-neutral-500 w-full max-w-[350px]">{route.path}</p>
                </div>
              </div>
            ))}
          </div>
